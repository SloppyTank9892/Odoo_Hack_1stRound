...
export default function ThankYouPage() {
  ...
  const totalCost = calculateTripTotalCost(activeTrip);
  ...
  return (
    <AppShell>
      <div className="max-w-3xl mx-auto py-6 sm:py-12 px-4 text-center">
        ...
        {/* Success Icon */}
        <div className="w-16 h-16 rounded-3xl bg-[#EDF7F2] dark:bg-[#102318] border border-[#A3D9BE] dark:border-[#1E4B33] text-[#1B8755] dark:text-[#34D399] flex items-center justify-center mx-auto mb-6 shadow-sm">
        ...
        <div className="bg-white dark:bg-[#1C1B18] rounded-3xl border border-[#E7E2D8] dark:border-[#33302B] p-6 sm:p-8 text-left mb-8 shadow-xs max-w-xl mx-auto transition-colors">
          <div className="flex items-center justify-between border-b border-[#E7E2D8] dark:border-[#33302B] pb-4 mb-4">
            ...
            <span className="text-xs font-bold text-[#1B8755] dark:text-[#34D399] bg-[#EDF7F2] dark:bg-[#132D20] px-2.5 py-1 rounded-full border border-[#A3D9BE]/50 dark:border-[#1E4B33]">
              Live & Synced
            </span>
          </div>
          ...
          <div className="space-y-2.5 text-xs text-[#6B655E] dark:text-[#A8A196] mb-6">
            <div className="flex items-center gap-2">
              ...
              <span>
                <strong className="text-[#181818] dark:text-[#F5F3EF]">Route:</strong> {activeTrip.stops.map((s) => s.cityName).join(" → ")}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-[#76546F] dark:text-[#B88BAF]" />
              <span>
                <strong className="text-[#181818] dark:text-[#F5F3EF]">Duration:</strong> {activeTrip.days.length} Days ({activeTrip.startDate} to{" "}
                {activeTrip.endDate})
              </span>
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-[#E7E2D8]/60 dark:border-[#33302B] text-xs">
          ...
          <div className="flex flex-wrap gap-2.5">
            <Link href={`/trips/${activeTrip.id}`} className="flex-1">
              <Button size="md" className="w-full text-xs" rightIcon={<ArrowRight className="w-4 h-4" />}>
                Open Workspace
              </Button>
            </Link>
            <Link href={`/share/${activeTrip.id}`} className="flex-1">
              <Button
                variant="outline"
                size="md"
        ...
        <div className="flex items-center justify-center gap-4 text-xs font-semibold">
          ...
          <span className="text-[#D5CEBF] dark:text-[#48443D]">·</span>
