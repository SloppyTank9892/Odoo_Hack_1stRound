...
export default function PublicStoriesPage() {
  ...
  return (
    <AppShell>
      {/* Header Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-[#181818] text-white p-6 sm:p-10 mb-8 border border-[#2D2A26] shadow-md">
      ...
      {isLoading ? (
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-[#76546F]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredStories.map((story) => {
              ...
              return (
                <Card
                  key={story.id}
                  className="overflow-hidden p-0 flex flex-col justify-between group hover:shadow-md transition-all duration-300 border-[#E7E2D8]"
                  hover
                >
                  {/* Top Cover Image */}
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between bg-white">
                    {/* Route Breadcrumb */}
                    <div className="mb-4">
                      <span className="block text-[10px] uppercase font-bold text-[#9E978E] tracking-wider mb-1.5">
                        Route Destinations
                      </span>
                      <div className="flex flex-wrap items-center gap-1.5 text-xs font-semibold text-[#181818]">
                        {story.cities_preview.length > 0 ? (
                          story.cities_preview.slice(0, 3).map((city, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#FAF9F5] rounded-md border border-[#E7E2D8] text-[11px]"
                            >
                          ...
                          <span className="text-[#9E978E] text-[11px]">Custom Journey</span>
                        )}
                        {story.cities_preview.length > 3 && (
                          <span className="text-[10px] text-[#76546F] font-bold">
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        {story.author_avatar ? (
                          <img
                            ...
                            className="w-6 h-6 rounded-full object-cover border border-[#E7E2D8]"
                          />
                        ) : (
                        ...
                        <span className="text-xs text-[#6B655E] truncate font-medium">
                          {story.author_name}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <Link href={`/share/${story.id}`}>
                          <button
                            ...
                            className="p-1.5 rounded-lg text-[#6B655E] hover:text-[#181818] hover:bg-[#FAF9F5] border border-transparent hover:border-[#E7E2D8] transition-colors"
                          >
        ...
        <Card className="text-center py-16 px-6 bg-white max-w-lg mx-auto border-[#E7E2D8]">
          ...
          <p className="text-xs sm:text-sm text-[#6B655E] mb-6 max-w-xs mx-auto">
            {searchQuery
